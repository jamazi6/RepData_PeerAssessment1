...
```{r Read in Data, echo=TRUE, results='hide', message=FALSE}
library(knitr)
library(ggplot2)
library(dplyr)
library(lattice)
...
```{r Aggregate by Day, include=TRUE, echo=TRUE}
activityByDate <- aggregate(activity[c("steps")], by=list(date=activity$date), sum)
```
```{r Plot By Day, include=TRUE, echo=TRUE}
...
```{r Calculate Mean and Median, include=TRUE, echo=TRUE}
...
```{r Aggregate By Time, include=TRUE, echo=TRUE}
...
```{r Max Steps for Interval, include=TRUE, echo=TRUE}
...
```{r Replace NAs, include=TRUE, echo=TRUE}
...
```{r Histogram with NAs Replaced, include=TRUE, echo=TRUE}
...
```{r New Mean and Median, include=TRUE, echo=TRUE}
...
```{r Determine Weekdays, include=TRUE, echo=TRUE}
...
```{r Plot Weekend Weekday Data, include=TRUE, echo=TRUE}
activity_na_rm <- activity[!is.na(activity$steps),] #remove NA values
